...
- [ ] 2.2 Mock 데이터 생성기 구현 (`src/lib/data/mock-generator.ts`)
    ...
    - [ ] 2.2.2 100개 랜덤 배차 콜(Order) 생성 엔진
        ...
        - [ ] 2.2.2.3 `generateMockOrders(count: number)` 메인 함수 구현 (Order 객체 100개 생성)
    - [ ] 2.2.3 데이터 유효성 검증 및 시드 관리
        - [ ] 2.2.3.1 생성된 100개 오더의 무게/부피 합계 및 제약 조건 1차 검증 테스트
        - [ ] 2.2.3.2 일관된 테스트를 위한 랜덤 시드(Seed) 기반 생성 옵션 추가 (Optional)

## 3. 시뮬레이션 엔진 및 알고리즘 프로토타입 (`src/lib/engine`)
- [ ] 3.1 거리 및 운임 계산 로직
    - [ ] 3.1.1 하버사인 거리 및 기본 운임 엔진
        - [ ] 3.1.1.1 `PRICING_MODEL.md` 기반의 기본 배송 요금 산정 함수 구현
- [ ] 3.2 합짐(Bundling) 알고리즘 1차 구현
    - [ ] 3.2.1 제약 조건 검증 로직
        - [ ] 3.2.1.1 1톤 적재량 및 시간 윈도우(Hard Constraints) 체크 함수 구현
    - [ ] 3.2.2 클러스터링 기반 합짐 시뮬레이터
        - [ ] 3.2.2.1 `ALGORITHM_SPEC.md`의 스코어링 함수($Score$)를 적용한 기초 합짐 엔진

## 4. UI/UX 구현 및 시각화 (`src/features/simulator`)
- [ ] 4.1 지도 시각화 레이어 구현
    - [ ] 4.1.1 네이버 지도 베이스 컴포넌트 (`components/naver-map.tsx`)
        - [ ] 4.1.1.1 `NaverMap` 컴포넌트 구현 및 서울 중심부 (`37.5665, 126.9780`) 초기화
        - [ ] 4.1.1.2 `useNaverMap` 커스텀 훅을 통한 지도 인스턴스 관리 로직 분리
    - [ ] 4.1.2 오더 마커 및 경로 렌더링
        - [ ] 4.1.2.1 `MOCK_DATA_SPEC.md` 규격에 맞춘 상차(주황색), 하차(파란색) 커스텀 마커 구현
        - [ ] 4.1.2.2 `Polyline`을 활용한 개별 배송(남색, 50% 투명도) 및 합짐(에메랄드색) 경로 렌더링
...
- [ ] 4.3 시뮬레이션 제어 및 상태 관리
    - [ ] 4.3.1 제어 패널 (`components/control-panel.tsx`)
        ...
        - [ ] 4.3.1.2 `Zustand` 또는 `React Context`를 활용한 전역 시뮬레이션 상태 동기화
